...
import { AxisDirection, FieldMap, Ship } from "common/type";
...
interface IPlaceShipByDisposition { shipDisposition: FieldMap, fieldNode: HTMLElement, clearFieldNode?: boolean }

export function placeShipByDisposition({ shipDisposition, fieldNode, clearFieldNode = true }: IPlaceShipByDisposition) {
    const fieldCells = Array.from(fieldNode.querySelectorAll('.ship-field-cell')) as HTMLDivElement[]
    ...
    shipDisposition.forEach((rowArray, yIndex) => {
        rowArray.forEach((value, xIndex) => {
            const cell = fieldCells.find(cell => cell.dataset.shipFieldCoord === `${xIndex + 1}-${yIndex + 1}`)
